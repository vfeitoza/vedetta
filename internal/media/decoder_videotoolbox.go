...
static OSStatus createDecompSession(CMVideoFormatDescriptionRef formatDesc,
    ...
    // Explicitly request hardware-accelerated decode. "Enable" (not "Require")
    // lets VideoToolbox use its own optimized fallback for the rare profile a
    // given Mac cannot hardware-decode, which is still cheaper than routing back
    // to the Go software path. On Apple Silicon H.264 is always hardware-decoded.
    ...
    OSStatus status = VTDecompressionSessionCreate(
        kCFAllocatorDefault, formatDesc, spec, attrs, &cb, outSession);
    CFRelease(attrs);
    CFRelease(spec);
    return status;
}
...
func (d *vtDecoder) Decode(nalData []byte) *image.YCbCr {
	...
	// splitAnnexB (shared with the software path) handles both 3- and 4-byte
	// start codes, so this stays correct regardless of how upstream framed the AU.
	nalus := splitAnnexB(nalData)
	if len(nalus) == 0 {
		return nil
	}
...
func (d *vtDecoder) createSession() bool {
	...
	if status != 0 {
		...
		// Clear the just-released ref so a later Close/createSession never sees a
		// dangling format description. CoreMedia ref types are not assignable from
		// untyped nil, so use the typed zero value.
		var zeroFmt C.CMVideoFormatDescriptionRef
		d.formatDesc = zeroFmt
		d.hasSession = false
		return false
	}
	d.hasSession = true
