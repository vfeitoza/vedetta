...
static AVBufferRef* vaapi_create_hwdevice(const char *device) {
    AVBufferRef *hw_device_ctx = NULL;
    if (av_hwdevice_ctx_create(&hw_device_ctx, AV_HWDEVICE_TYPE_VAAPI, device, NULL, 0) < 0) {
        return NULL;
    }
...
static void vaapi_set_get_format(AVCodecContext *ctx) {
    ctx->get_format = vaapi_get_format;
}
*/
import "C"
...
func probeVAAPI() bool {
	for _, dev := range []string{"/dev/dri/renderD128", "/dev/dri/renderD129"} {
		if _, err := os.Stat(dev); err != nil {
			continue
		}
		cdev := C.CString(dev)
		ctx := C.vaapi_create_hwdevice(cdev)
...
func newVAAPIDecoder() (*vaapiDecoder, error) {
	...
	for _, dev := range []string{"/dev/dri/renderD128", "/dev/dri/renderD129"} {
		...
		cdev := C.CString(dev)
		hwCtx = C.vaapi_create_hwdevice(cdev)
		C.free(unsafe.Pointer(cdev))
		if hwCtx != nil {
			break
		}
	...
	if hwCtx == nil {
		return nil, errors.New("vaapi: no usable DRI render node")
	}

	codec := C.avcodec_find_decoder(C.AV_CODEC_ID_H264)
	if codec == nil {
		C.av_buffer_unref(&hwCtx)
		return nil, errors.New("vaapi: h264 decoder not found")
	}

	codecCtx := C.avcodec_alloc_context3(codec)
	if codecCtx == nil {
		C.av_buffer_unref(&hwCtx)
		return nil, errors.New("vaapi: alloc context failed")
	}

	codecCtx.hw_device_ctx = C.av_buffer_ref(hwCtx)
	C.vaapi_set_get_format(codecCtx)

	if C.avcodec_open2(codecCtx, codec, nil) < 0 {
		C.avcodec_free_context(&codecCtx)
		C.av_buffer_unref(&hwCtx)
		return nil, errors.New("vaapi: open codec failed")
	}
...
func (d *vaapiDecoder) Decode(nalData []byte) *image.YCbCr {
	if len(nalData) == 0 {
		return nil
	}
...
func (d *vaapiDecoder) Flush() *image.YCbCr {
...
func (d *vaapiDecoder) extractNV12() *image.YCbCr {
