...
func TestParseHWAccel(t *testing.T) {
	cases := []struct {
		...
		{"vaapi", HWAccelVAAPI, true}, // valid value; backend present only in -tags hwaccel builds
		{"nvdec", HWAccelNVDEC, true}, // valid value; backend present only in -tags hwaccel builds
